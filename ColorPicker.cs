...
public class ColorPicker : MonoBehaviour
{
    ...
    {   
        if(instance is null)
        {
            Debug.LogError("<color=red>[Color Picker Error]</color> ColorPicker instance is null. Is the prefab in the scene?");
            return false;
        }
        ...
        if(done)
        {
            ...
            // Safe title lookup to avoid brittle hierarchy assumptions.
            Text titleText = instance.GetComponentInChildren<Text>(true);
            if (titleText != null)
            ...
            {
                Text placeholderText = instance.hexaComponent.placeholder.GetComponent<Text>();
                if (placeholderText != null)
                    placeholderText.text = "RRGGBB" + (useAlpha ? "AA" : "");
            }
    ...
    {
        Debug.Log($"<color=orange>[Color Picker]</color> Modified Color is now: {modifiedColor}");
        ...
        Color finalColor = modifiedColor;

        if (colorComponent != null)
        {
            // Keep preview visible when alpha is disabled.
            if (finalColor.a == 0 && !useA)
                finalColor.a = 1f;

            colorComponent.color = finalColor;

            // If a custom material is used, explicitly drive its tint too.
            Material pickerMaterial = colorComponent.material;
            if (pickerMaterial != null)
            {
                if (pickerMaterial.HasProperty("_Color"))
                    pickerMaterial.SetColor("_Color", finalColor);

                if (pickerMaterial.HasProperty("_BaseColor"))
                    pickerMaterial.SetColor("_BaseColor", finalColor);
            }

            modifiedColor = finalColor;
            Debug.Log($"<color=orange>[Color Picker]</color> UI Component Color set to: {finalColor}");
        }
        else
        {
            Debug.LogError("<color=red>[Color Picker Error]</color> ColorComponent RawImage is MISSING in ColorPicker Inspector!");
        }

        onCC?.Invoke(finalColor);
        interact = true;
    }
